...
exports.createExpense = async (req, res) => {
  const { title, amount, description } = req.body;
    ...
    try {
      ...
      if (!title || !amount) {
        return res.status(400).json({ message: "Title and amount fields cannot be empty" });
      }
      ...
      const titleStatus = await findTitleInCategory(user, title);
        if (titleStatus.length === 0) {
            return res.status(404).json({ message: `${title} category not found. Kindly add it up.` });
        };
